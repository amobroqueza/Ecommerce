...
'</h2>';
            echo '<p>Price: P. $row['prod_price']
.'</p>';
            echo '<button><a

href="order_form.php?product_id='. $row['prod_id'].">Buy Now</a></button>';
            echo '</div>';
        }
    }
...
method="post">;
echo '<table>'; 
echo '<tr><th>Product</th><th>Quantity</th> 
<th>Price</th></tr>';

while ($row = mysqli_fetch_assoc($result)) {
    echo '<tr>'; echo '<td>' . $row["prod_name"] . '</td>";
...
<?php
// View the orders
$sql = "SELECT o.name, o.address, o.email, 
o.total, o.status, o.date_purchased_ts, o.order_id,
            od.quantity,
...
<head>
    <meta charset="UTF-8">
    <title>View Order</title> <link rel="stylesheet" href="style.css">
